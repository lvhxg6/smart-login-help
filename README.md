...
- 用户名和密码使用主密码派生的 AES-GCM 密钥加密后存储。
- 主密码不会保存；浏览器重启后需要重新解锁加密缓存。
...
5. 输入主密码并点击“确定保存”后，下次进入当前站点会自动填充用户名和密码。

## 安全说明

账号密码不会以明文字段保存。插件会用主密码通过 PBKDF2-SHA256 派生 AES-GCM 密钥，然后把用户名和密码作为密文写入 `chrome.storage.local`。

主密码本身不会落盘。浏览器会话中会临时保存派生密钥，浏览器重启后需要在插件弹窗的“安全解锁”中重新输入主密码。

如果你使用过旧版本的明文缓存，可以在“安全解锁”里输入主密码并点击“启用加密”，插件会把已有明文凭据迁移成密文并删除明文字段。
